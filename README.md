...
- **Multiple embedding models**: Support for ESM-C (300M), E1 (150M/300M), ESM3, and ProtT5-XL protein language models
...
- **ProtT5-XL embeddings**: Per-residue embeddings from the ProtT5-XL-UniRef50 encoder (1024D)
- **Masked sequence analysis**: Create embeddings with specific residue positions masked for mutation effect studies (ESM-C/ESM3)
...
E1 requires Python 3.12. Create a separate virtual environment:
...
### Using ProtT5 Embeddings

ProtT5 depends on `transformers`, `sentencepiece`, and `protobuf`, which can conflict with the ESM and E1 dependency versions. Create a separate virtual environment:

```bash
python3 -m venv venv_prott5
source venv_prott5/bin/activate  # On macOS/Linux
# venv_prott5\Scripts\activate  # On Windows

# Install modelomics + ProtT5 dependencies
pip install git+https://github.com/alanclayrichard/modelomics.git
...
# Generate ProtT5-XL embeddings (requires separate venv)
embedding_prott5 = sequence_embeddings.embed_sequence(seq_pdb, model="prott5")
print("ProtT5 embedding shape:", embedding_prott5.shape)  # [seq_len, 1024]

# Create masked embeddings for mutation analysis (ESM-C/ESM3 only)
